...
async function main() {
  ...
  try {
    ...
    console.log(
      '   If PII is currently plain text, this step will encrypt it for the first time\n',
    );
    ...
    for (const rb of residentBorrowers) {
      try {
        ...
        const reEncryptedPhone = decryptedPhone
          ? encryptWithEntityKey(decryptedPhone, entityKey)
          : null;
    ...
    for (const eb of externalBorrowers) {
      try {
        ...
        const reEncryptedPhone = decryptedPhone
          ? encryptWithEntityKey(decryptedPhone, entityKey)
          : null;
        const reEncryptedAddress = decryptedAddress
          ? encryptWithEntityKey(decryptedAddress, entityKey)
          : null;
        const reEncryptedCompany = decryptedCompany
          ? encryptWithEntityKey(decryptedCompany, entityKey)
          : null;
        const reEncryptedPurpose = decryptedPurpose
          ? encryptWithEntityKey(decryptedPurpose, entityKey)
          : null;
    ...
    const verificationChecks = {
      ...
      issueRecordsWithEntity: await prisma.issueRecord.count({
    ...
    console.log(
      `   - Borrowers with entity: ${verificationChecks.borrowersWithEntity}/${borrowers.length}`,
    );
    console.log(
      `   - KeyTypes with entity: ${verificationChecks.keyTypesWithEntity}/${keyTypes.length}`,
    );
    console.log(
      `   - IssueRecords with entity: ${verificationChecks.issueRecordsWithEntity}/${issueRecords.length}\n`,
    );
